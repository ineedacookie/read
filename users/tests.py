...
from .forms import CustomUserCreationForm, CompanyForm, UserForm
from .utils import check_employee_form
from .tokens import account_activation_token
from .models import Company, CustomUser, CompanyConnection, TTCompanyInfo, TTUserInfo
...
class CompanyModelTests(TestCase):
    ...
    def test_company(self):
        ...
        for i in range(num_emps):
            new_user = CustomUser.objects.create(email=(str(i) + '@gmail.com'), password='foo', first_name=str(i),
                                                 last_name=str(i), company=new_com)
...
class ModelsTest(TestCase):
    ...
    def test_user_with_company(self):
        ...
        test_user = CustomUser.objects.create(first_name='ted', last_name='Great', email="great@ted.com",
...
class FormTests(TestCase):
    ...
    def test_customusercreationform(self):
        form = CustomUserCreationForm(
            data={'email': 'junk@gmail.com', 'password1': 'hawkeye200', 'password2': 'hawkeye200'})
        ...
        self.assertEqual(form.errors, {'email': ['User with this Email already exists.'], 'password2': [
    ...
    def test_companyform(self):
        ...
        self.assertEqual(form.errors,
                         {'timezone': ['Select a valid choice. AAAAAA is not one of the available choices.']})
        with self.assertRaises(ValueError):
            form.save()
    ...
    def test_userform(self):
        ...
        form = UserForm(data={'first_name': 'golopogose', 'last_name': 'bar', 'email': 'bar@gmail.com'},
                        instance=temp_emp)
        self.assertTrue(form.is_valid())
        form.save()
        self.assertEqual(CustomUser.objects.get(email='bar@gmail.com').first_name, 'golopogose')
        form = UserForm(data={'first_name': '', 'last_name': '', 'middle_name': 'a' * 51, 'email': 'fakeemailsee'},
                        instance=temp_emp)
        self.assertFalse(form.is_valid())
...
class UtilsTests(TestCase):
    ...
    def test_check_employee_form(self):
        ...
        form = UserForm(data={'first_name': 'Stan', 'last_name': 'Pinesworth', 'email': initial_email,
                              'timezone': 'America/Denver'}, instance=self.employee)
        response = check_employee_form(current_site, form, initial_email, send=False)
        self.assertTrue(response['success'])
        self.assertFalse(response['change_email'])
        form = UserForm(data={'first_name': 'Stan', 'last_name': 'Pines', 'email': 'stanford@pineswoods.com',
